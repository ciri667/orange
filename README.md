...
Cici Note 是一款面向个人知识工作的本地优先 Agent 笔记应用。它将本地 Markdown 文件夹视为用户拥有的知识库，并在熟悉的桌面笔记工作台中，让助手完成查找、问答、改写、生成草稿和整理知识等操作。
...
- 桌面端使用 Tauri v2 和 Rust，`src-tauri/` 负责本地目录选择、Markdown 扫描、SQLite/FTS5 索引、规则 Agent 模拟和安全写入确认。
- 目标架构中 Agent 会以 loop 方式运行，检索被建模为 `search_notes` 工具；当前实现仍是规则模拟，后续会替换为真实模型驱动的 Runtime。
